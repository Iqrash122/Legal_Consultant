...
const Lawyer = ({ item }: { item: Record<string, any> }) => {
  ...
  return (
    <div className="">
      <div
        className={`fixed ${
          visible ? "top-0" : "top-[100vh]"
        } transition-all duration-300 left-0 bg-[#fffbd8] bg-opacity-60 overflow-y-scroll h-screen w-screen z-50`}
      >
        <div className="overflow-y-scroll h-screen w-[90%] max-[960px]:w-full mx-auto shadow bg-[#fffbd8] p-24 max-[1130px]:px-12 max-[450px]:px-6 relative">
          <div className="flex gap-12 max-[890px]:block">
            ...
            <div
              className="max-[890px]:w-full-890"
              style={{ width: "calc(100% - 320px)" }}
            >
              <div className="text-4xl max-[450px]:text-3xl max-[890px]:text-center max-[890px]:mt-3">
                {item.name}
              </div>
              <div className="text-2xl max-[450px]:text-xl max-[890px]:text-center  text-[#222] font-bold mb-3 mt-1.5">
                {item.title}
              </div>
              ...
              ></div>
              <a
                href="http://localhost/Laweyr_Appointment/"
                target="_blank"
                className="px-6 py-4 bg-[#e88b28] text-white capitalize mt-3 inline-block text-center hover:shadow-lg transition-shadow duration-300"
                >
                Book Appointment
              </a>
            </div>
          ...
          <div className="absolute top-0 right-0 py-6 px-12">
            <div
              ...
              className="h-[50px] w-[50px] rounded-full border flex items-center justify-center"
            >
              <X />
            </div>
          </div>
